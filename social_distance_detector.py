...
while True:
    ...
    if frame_skip:
        ...
        print("[INFO] frame " + str(iteration))
        ...
        # generate heatmap using py/map/heatmapper
        heatmap, (average, zones_x, zones_y) = heatmapper.getMap(all_centroids, frame, iteration)

        # generate flowmap using py/map/flowmapper
        flowmap = flowmapper.getMap(all_centroids)

        if iteration > 10:
            safest_centroid = pathfind.getSafestPerson(non_violate_centroids, average, zones_x, zones_y)
            cv2.rectangle(heatmap, (safest_centroid[0] * config.BLOCKSIZE_X, safest_centroid[1] * config.BLOCKSIZE_Y),
